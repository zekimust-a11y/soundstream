...
class RoonVolumeControl {
  ...
  constructor(config: RoonVolumeControlConfig) {
    ...
    this.extensionId = 'com.soundstream.roon.volume.v2';
  }
  ...
  async setVolume(volume: number): Promise<void> {
    ...
    console.log(`[RoonVolumeControl] Transport available: ${!!this.transport}`);
    console.log(`[RoonVolumeControl] Current output ID: ${this.currentOutputId}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(`[RoonVolumeControl] Current zone ID: ${this.currentZoneId}`);
      console.log(`[RoonVolumeControl] Zone found: ${!!zone}`);
      console.log(`[RoonVolumeControl] Available zones:`, Array.from(this.zones.keys()));

      if (zone) {
        console.log(`[RoonVolumeControl] Trying zone-based volume control first`);
        console.log(`[RoonVolumeControl] Zone details:`, zone);
        console.log(`[RoonVolumeControl] Calling change_volume with zone_id: ${zone.zone_id}, how: absolute, value: ${targetValue}`);

        // Add timeout to prevent hanging
        const timeout = setTimeout(() => {
          console.log(`[RoonVolumeControl] Zone change_volume timed out after 5 seconds, trying output control`);
          clearTimeout(timeout);
          // Fallback to output-based control on timeout
          this.tryOutputVolumeControl(targetValue, resolve, reject);
        }, 5000);

        // Try with correct Roon API format: control_key, how, value
        console.log(`[RoonVolumeControl] Trying zone change_volume with control_key: {zone_id}, how=absolute, value=${targetValue}`);
  ...
  private tryOutputVolumeControl(targetValue: number, resolve: () => void, reject: (error: Error) => void) {
    ...
    console.log(`[RoonVolumeControl] Current output ID: ${this.currentOutputId}`);
    console.log(`[RoonVolumeControl] Calling change_volume with output_id: ${this.currentOutputId}, how: absolute, value: ${targetValue}`);

    // Add timeout to prevent hanging
    const timeout = setTimeout(() => {
      console.log(`[RoonVolumeControl] Output change_volume timed out after 5 seconds`);
      reject(new Error('Output volume change timed out'));
    }, 5000);

    // Try with object format as the Roon API might expect it
    console.log(`[RoonVolumeControl] Trying change_volume with object format: {output_id: ${this.currentOutputId}}, how=absolute, value=${targetValue}`);
    this.transport!.change_volume(
  ...
  async volumeUp(step: number = 2): Promise<number> {
    console.log(`[RoonVolumeControl] volumeUp called with step: ${step}`);
    const current = await this.getVolume();
    console.log(`[RoonVolumeControl] current volume: ${current}`);
    const newVolume = Math.min(100, current + step);
    console.log(`[RoonVolumeControl] new volume will be: ${newVolume}`);
    console.log(`[RoonVolumeControl] calling setVolume...`);
    await this.setVolume(newVolume);
    console.log(`[RoonVolumeControl] setVolume completed`);
    return newVolume;
  }
   ...
   */
  isReady(): boolean {
    // Check if we have a paired core (indicates authorization)
    const hasPairedCore = (this.roon as any).paired_core !== undefined;

    // Check if we have outputs with volume control
    const hasVolumeOutput = Array.from(this.outputs.values()).some(output => output.volume);

    // Check if we have a current output selected
    const hasCurrentOutput = this.currentOutputId && this.outputs.has(this.currentOutputId);

    const ready = this.isConnected && hasPairedCore && hasVolumeOutput && hasCurrentOutput;
    console.log(`[RoonVolumeControl] isReady() called: isConnected=${this.isConnected}, hasPairedCore=${hasPairedCore}, hasVolumeOutput=${hasVolumeOutput}, hasCurrentOutput=${hasCurrentOutput}, ready=${ready}`);
    return ready;
  }
