...
export default function BrowseScreen() {
  ...
  useEffect(() => {
    const loadTidalPlaylists = async () => {
      ...
      try {
        const { getApiUrl } = await import('@/lib/query-client');
        const response = await fetch(`${getApiUrl()}api/tidal/playlists?limit=30`);
        if (response.ok) {
          const data = await response.json();
          const playlists = (data.items || []).map((playlist: any) => ({
            id: `tidal-playlist-${playlist.id}`,
            name: playlist.title,
            url: playlist.lmsUri || `tidal://playlist:${playlist.id}`,
            artwork_url: playlist.cover,
            trackCount: playlist.numberOfTracks || 0,
            source: 'tidal' as const,
          }));
          setTidalPlaylists(playlists);
        } else {
          console.warn('Failed to fetch Tidal playlists:', response.status);
          setTidalPlaylists([]);
        }
      } catch (e) {
        debugLog.info(
          "Tidal playlists not available",
          e instanceof Error ? e.message : String(e),
        );
        setTidalPlaylists([]);
      }
