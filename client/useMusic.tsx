...
export function MusicProvider({ children }: { children: ReactNode }) {
  ...
  const getTidalAuthUrl = useCallback(async (): Promise<string> => {
    // For now, return a mock Tidal auth URL that the user can manually use
    // In a real implementation, this would call the server
    const clientId = 'pUlCxd80DuDSem4J';
    const redirectUri = encodeURIComponent('soundstream://callback');
    return `https://login.tidal.com/authorize?response_type=code&client_id=${clientId}&redirect_uri=${redirectUri}&scope=r_usr+w_usr+w_sub`;
  }, []);
  ...
  const connectTidal = useCallback(async (authCode?: string): Promise<boolean> => {
    try {
      ...
      // In a real implementation, this would exchange the code for tokens via server
      if (authCode) {
