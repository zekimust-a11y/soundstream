  ...
  const playTrack = useCallback(async (track: Track, tracks?: Track[]) => {
    ...
    try {
      if (track.lmsTrackId || track.uri) {
        ...
        // Handle Tidal tracks using URI (lmsUri or uri)
        const tidalUri = track.source === 'tidal' ? (track.lmsUri || track.uri) : null;
        
        if (tidalUri) {
          debugLog.info('Playing Tidal track via URL', `URI: ${tidalUri}`);
          
          // If we have a queue of tracks, load all of them into the playlist
          if (tracks && tracks.length > 0) {
            debugLog.info('Loading Tidal queue into playlist', `${tracks.length} tracks`);
            ...
            for (let i = 0; i < tracks.length; i++) {
              const t = tracks[i];
              if (t.source === 'tidal' && (t.lmsUri || t.uri)) {
                // Tidal track - use URL
                const uri = t.lmsUri || t.uri;
                await lmsClient.addUrlToPlaylist(activePlayer.id, uri);
              } else if (t.lmsTrackId) {
                // LMS track - use track ID
            ...
            // No queue provided, just play the single Tidal track
            await lmsClient.playUrl(activePlayer.id, tidalUri);
            await lmsClient.play(activePlayer.id);
          }
        } else if (track.lmsTrackId) {
          // If we have a queue of tracks, load all of them into the playlist
          if (tracks && tracks.length > 0) {
