...
export default function SettingsScreen() {
  ...
  const { servers, qobuzConnected, tidalConnected, refreshLibrary, clearAllData, isLoading, addServer, activeServer, removeServer, playlists, getTidalAuthUrl, connectTidal, disconnectTidal, checkTidalStatus, qobuzEnabled, tidalEnabled, soundcloudEnabled, spotifyEnabled } = useMusic();
  ...
  const loadLibraryStats = async () => {
    try {
      ...
      console.log("Calling lmsClient.getLibraryTotals()");
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.sectionContent, { backgroundColor: theme.backgroundDefault }]}>
            <View style={[styles.serverRow, { borderColor: theme.border }]}>
              <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                <View style={[styles.serverIcon, { backgroundColor: '#F99C38' + '20' }]}>
                  <Feather name="headphones" size={16} color="#F99C38" />
                </View>
                <View style={styles.serverInfo}>
                  <ThemedText style={[styles.serverName, { color: theme.text }]}>
                    Qobuz
              ...
              <Switch
                value={qobuzEnabled}
                onValueChange={setQobuzEnabled}
                trackColor={{ false: theme.border, true: theme.accent + '40' }}
                thumbColor={qobuzEnabled ? theme.accent : theme.textTertiary}
              />
            </View>
            <View style={[styles.serverRow, { borderColor: theme.border }]}>
              <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View style={styles.serverInfo}>
                  ...
                  <ThemedText style={[styles.serverAddress, { color: theme.textSecondary }]}>
                    {tidalConnected ? 'Connected to Tidal API' : 'Available via LMS plugin'}
                  </ThemedText>
                </View>
              </View>
                  ...
                  <Pressable
                    ...
                    onPress={async () => {
                      try {
                        ...
                        // Show auth URL and ask for auth code
                        Alert.alert(
                          'Tidal Authentication',
                          '1. Copy this URL and paste it in your browser:\n\n' + authUrl + '\n\n2. Login to Tidal and authorize the app\n\n3. You will be redirected to a URL with a code parameter\n\n4. Copy the code and enter it below',
                          [
                            { text: 'Cancel', style: 'cancel' },
                            {
                              text: 'Enter Code',
                                        ...
                                        if (code) {
                                          try {
                                            ...
                                            if (success) {
                                              ...
                                              Alert.alert('Error', 'Failed to connect to Tidal');
                                            }
                                          } catch (error) {
                                            Alert.alert('Error', 'Failed to connect to Tidal');
                                          }
                        ...
                        Alert.alert('Error', 'Failed to get Tidal authentication URL');
                      }
                    }}
                  >
