...
class LmsClient {
  ...
  private async request(playerId: string, command: string[]): Promise<Record<string, unknown>> {
    ...
    if (Platform.OS === 'web') {
      try {
        ...
        let protocol = this.serverProtocol;
        
        if (!host && this.baseUrl) {
          // Extract host, port, and protocol from baseUrl
          try {
            ...
            if (urlMatch) {
              protocol = urlMatch[1] as 'http' | 'https';
              host = urlMatch[2];
              port = urlMatch[3] ? parseInt(urlMatch[3], 10) : (protocol === 'https' ? 443 : 9000);
            }
          }
        }
        ...
        const response = await fetch(`${apiUrl}/api/lms/proxy`, {
          ...
          credentials: 'include',
          body: JSON.stringify({
            url: this.baseUrl, // Send full URL for remote access support
            host,
            port,
            protocol,
            playerId,
        ...
        if (error instanceof Error && error.name === 'AbortError') {
          debugLog.error('LMS request timeout', 'Request took longer than 10 seconds');
  ...
  async getAlbumsPage(start: number = 0, limit: number = 50, artistId?: string): Promise<{ albums: LmsAlbum[], total: number }> {
    ...
    debugLog.info('getAlbumsPage', `Received ${albumsLoop.length} albums, total=${total}`);
    
    const albums = albumsLoop.map((a) => {
      // Check if this is Tidal content by examining URL or other fields
      const url = String(a.url || '').toLowerCase();
      const id = String(a.id || '').toLowerCase();
      const artworkUrl = String(a.artwork_url || '').toLowerCase();
      const isTidal = url.includes('tidal') || id.includes('tidal') || artworkUrl.includes('tidal');
      ...
      return {
        ...
        url: a.url ? String(a.url) : undefined, // Include URL for source detection
      };
    });
  ...
  async getPlaylists(includeQobuz: boolean = false, includeSoundCloud: boolean = false, includeSpotify: boolean = false, includeTidal: boolean = false): Promise<LmsPlaylist[]> {
    ...
    if (playlistsLoop && playlistsLoop.length > 0) {
      allPlaylists.push(...playlistsLoop.map((p) => ({
        id: String(p.id || ''),
  ...
  getArtworkUrl(track: LmsTrack | LmsAlbum): string | undefined {
    if ('artwork_url' in track && track.artwork_url) {
      ...
      if (track.artwork_url.startsWith('http')) {
        ...
        imageUrl = `${this.baseUrl}${track.artwork_url}`;
      }

      // Proxy images through the server to avoid CORS issues
  ...
  async getLibraryTotals(): Promise<{ albums: number; artists: number; tracks: number; radioStations: number; playlists: number }> {
    try {
      ...
      // Get local library counts
      let localAlbums = 0;
      let localTracks = 0;
      try {
        const albumsResult = await this.request('', ['info', 'total', 'albums', '?']);
        localAlbums = Number(albumsResult._albums || 0);
      } catch (e) {
        debugLog.info('Failed to get local albums count', e instanceof Error ? e.message : String(e));
      }
      
      try {
        const songsResult = await this.request('', ['info', 'total', 'songs', '?']);
        localTracks = Number(songsResult._songs || 0);
      } catch (e) {
        debugLog.info('Failed to get local tracks count', e instanceof Error ? e.message : String(e));
      }
    ...
    // Count Tidal albums and tracks from standard LMS library (Tidal content is included in standard queries)
    // Note: We fetch albums/tracks in batches to get accurate counts
    let tidalAlbums = 0;
    let tidalTracks = 0;
    try {
      // Fetch albums in batches to count Tidal albums
      // Even if info total albums returns 0, we should still try to fetch to see if there are Tidal albums
      ...
      while (hasMore) {
        ...
        for (const album of albumsLoop) {
          ...
          if (url.includes('tidal') || id.includes('tidal') || artworkUrl.includes('tidal')) {
            tidalAlbums++;
          }
        }
      ...
      debugLog.info('Tidal albums not available for counting', e instanceof Error ? e.message : String(e));
    }
    ...
    try {
      ...
      let playlistCount = 0;
      try {
        const playlists = await this.getPlaylists(true, true, true, true);
      ...
      const result = {
        albums: localAlbums + qobuzAlbums + tidalAlbums,
        artists: artistCount,
        tracks: localTracks + qobuzTracks + tidalTracks,
