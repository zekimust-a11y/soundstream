...
export default function DebugScreen() {
  ...
  const checkServerStatus = async () => {
    ...
    try {
      ...
      if (status) {
        ...
        debugLog.info('LMS server connected', `${status.name} - ${status.playerCount} players`);
      } else {
        setServerStatus('disconnected');
      }
    } catch (error) {
  ...
  const testGetArtists = async () => {
    ...
    try {
      const result = await lmsClient.getArtistsPage(0, 20);
      debugLog.response('Found artists', `${result.artists.length} artists:\n${result.artists.slice(0, 10).map(a => `- ${a.name}`).join('\n')}`);
    } catch (error) {
  ...
  const testGetAlbums = async () => {
    ...
    try {
      const result = await lmsClient.getAlbumsPage(0, 20);
      debugLog.response('Found albums', `${result.albums.length} albums:\n${result.albums.slice(0, 10).map(a => `- ${a.title} by ${a.artist}`).join('\n')}`);
    } catch (error) {
