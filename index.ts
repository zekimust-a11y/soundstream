...
function setupCors(app: express.Application) {
  app.use((req, res, next) => {
    ...
    // Allow remote IP addresses for network access
    origins.add("http://192.168.0.21:8081");
    origins.add("http://192.168.0.21:8082");
    origins.add("http://192.168.0.21:19006");
...
(async () => {
  try {
    ...
    // Force port 3000 and ensure clean startup
    const port = 3000;
    ...
    try {
      exec(`lsof -ti :${port} | xargs kill -9`, { timeout: 2000 });
    ...
    server.listen(port, "0.0.0.0", () => {
      log(`✅ SoundStream server running on http://192.168.0.21:${port}`);
      log(`✅ API health check: http://192.168.0.21:${port}/api/health`);
    }).on("error", (err: unknown) => {
      const error = err as NodeJS.ErrnoException;
      log(`❌ Failed to start server on port ${port}:`, error.message);
      log(`❌ Please check if another service is using port ${port}`);
      process.exit(1);
    });
  } catch (error) {
