...
class LmsClient {
  ...
  private async request(playerId: string, command: string[]): Promise<Record<string, unknown>> {
    ...
    if (Platform.OS === 'web') {
      try {
        ...
        const proxyUrl = `${apiUrl}/api/lms/proxy`;
  ...
  async getPlaylists(includeQobuz: boolean = false, includeSoundCloud: boolean = false, includeSpotify: boolean = false, includeTidal: boolean = false): Promise<LmsPlaylist[]> {
    ...
    if (includeSoundCloud) {
      ...
      if (playerId) {
        ...
        // Step 1: Browse Apps menu to find SoundCloud
        try {
          debugLog.info('getPlaylists', 'Trying to browse SoundCloud playlists via Apps menu');
          const appsResult = await this.request(playerId, ['browse', 'items', '0', '100', 'menu:apps']);
          const appsItems = (appsResult.items_loop || appsResult.item_loop || appsResult.items || []) as Array<Record<string, unknown>>;
          
          debugLog.info('getPlaylists', `Apps menu returned ${appsItems.length} items`);
          
          // Step 2: Find SoundCloud in Apps menu
          let soundcloudItemId: string | undefined;
          for (let i = 0; i < appsItems.length; i++) {
            const item = appsItems[i];
            const name = String(item.name || item.text || item.title || '').toLowerCase();
            if (name.includes('soundcloud')) {
              ...
              soundcloudItemId = actionItemId || paramItemId || (itemId || String(i));
              debugLog.info('getPlaylists', `Found SoundCloud app with id: ${soundcloudItemId}, name: ${name}`);
              break;
            }
          }
          
          if (soundcloudItemId) {
            // Step 3: Browse SoundCloud menu
            const soundcloudResult = await this.request(playerId, ['browse', 'items', '0', '100', `item_id:${soundcloudItemId}`, 'menu:apps']);
            ...
            debugLog.info('getPlaylists', `SoundCloud menu returned ${soundcloudItems.length} items`);
            
            // Step 4: Find "My Playlists" in SoundCloud menu
            let myPlaylistsId: string | undefined;
            for (let i = 0; i < soundcloudItems.length; i++) {
              const item = soundcloudItems[i];
              const name = String(item.name || item.text || item.title || '').toLowerCase();
              // Check for both "my playlist" and "my playlists" (singular and plural)
              if (name.includes('my playlist')) {
                ...
                myPlaylistsId = actionItemId || paramItemId || (itemId || String(i));
                debugLog.info('getPlaylists', `Found SoundCloud My Playlists with id: ${myPlaylistsId}, name: ${name}, index: ${i}`);
                break;
              }
            }
            
            if (myPlaylistsId !== undefined) {
              // Step 5: Get playlists from My Playlists
              const playlistsResult = await this.request(playerId, ['browse', 'items', '0', '100', `item_id:${myPlaylistsId}`, 'menu:apps']);
              const playlistItems = (playlistsResult.items_loop || playlistsResult.item_loop || playlistsResult.items || []) as Array<Record<string, unknown>>;
          ...
          } else if (appsItems.length > 0) {
            // If we couldn't find SoundCloud but got items, log them for debugging
            debugLog.info('getPlaylists', `Apps items (first 5): ${appsItems.slice(0, 5).map((i: Record<string, unknown>) => String(i.name || i.text || i.title || 'Unknown')).join(', ')}`);
          }
        } catch (browseError) {
  ...
  async discoverServer(hostOrUrl: string, port: number = 9000, timeoutMs: number = 3000): Promise<LmsServer | null> {
    ...
    if (Platform.OS === 'web') {
      try {
        // On web platform, use the current window location to determine the API server
        // This ensures we use the correct domain/IP (e.g., 192.168.0.21:3000) instead of localhost
        let domain: string;
        if (typeof window !== 'undefined') {
          // Use the same hostname as the web app, but port 3000 for the API server
